...
public class ANN {
    ...
    public List<double> Go(List<double> inputValues, List<double> desiredOutput) {
        ...
        for (int i = 0; i < numHidden + 1; i++) {
            ...
            for (int j = 0; j < layers[i].numNeurons; j++) {
                ...
                if(i == numHidden)                                          //if we are dealing with the output layer...
                    layers[i].neurons[j].output = ActivationFunctionO(N);       //use the activation fucntion specified for the output
                else                                                        //else if it is any other layer...
                    layers[i].neurons[j].output = ActivationFunction(N);        //use the regular activation function
    ...
    double ActivationFunctionO(double value) {//activateion function we are going to use for the output
